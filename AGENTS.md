...
- `RebaseEntryList` の `handleDragEnd` は `findIndex` の結果が `-1` の場合に `onReorder` を呼ばない。`onDragEnd` は `entries` を閉じ込めたコールバックで、ドラッグ中に一覧が差し替わると `active.id` / `over.id` が現在の `entries` に存在せず、`onReorder(-1, ...)` が並び順を壊すため
...
- `MergeEditor` のコンフリクト操作一覧（`ConflictActions` を行ごとに描画）は「未解決が 1 件以上」ではなく「コンフリクトが 1 件以上」を表示条件とする。全コンフリクト解決後も解決済み行の「戻す」ボタンを保持し、`revertConflict` が UI から到達不能にならないようにする（`ConflictActions` は唯一 `MergeEditor` からのみ描画される）
...
- `mergeStore.save` の保存中 `updateMergedContent` 競合（保存した内容と異なる追加入力で `isDirty: true` 維持、保存対象と同じ内容へ戻された場合は `isDirty: false`）をテストでカバー
...
- `RebaseEntryList` のマウス D&D 並び替え（下方向・上方向・隣接行・同一位置では非発火・8px 未満では非発火）と、各行が一覧コンテナの直接の子であること（wrapper 要素の再混入検知）、ドラッグ中に `entries` が差し替わった場合に `onReorder` を呼ばないことをテストでカバー
